...
```
% docker run --rm debian:wheezy bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150108-7153-13a1oww.out
% docker run --rm minimum2scp/wheezy-baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150108-7153-1xub8sx.out
% diff -u /tmp/docker-diff.20150108-7153-13a1oww.out /tmp/docker-diff.20150108-7153-1xub8sx.out
--- /tmp/docker-diff.20150108-7153-13a1oww.out	2015-01-08 23:52:18.921364737 +0900
+++ /tmp/docker-diff.20150108-7153-1xub8sx.out	2015-01-08 23:52:19.521358759 +0900
@@ -3,12 +3,18 @@
...
+ii  apt-utils                0.9.7.9+deb7u6    amd64             package managment related utility programs
 ii  base-files               7.1wheezy7        amd64             Debian base system miscellaneous files
...
+ii  curl                     7.26.0-1+wheezy11 amd64             command line tool for transferring data with URL synt
...
+ii  libcurl3:amd64           7.26.0-1+wheezy11 amd64             easy-to-use client-side URL transfer library (OpenSSL
+ii  libcurl3-gnutls:amd64    7.26.0-1+wheezy11 amd64             easy-to-use client-side URL transfer library (GnuTLS 
...
+ii  libpcre3:amd64           1:8.30-5          amd64             Perl 5 Compatible Regular Expression Library - runtim
...
+ii  locales                  2.13-38+deb7u6    all               Embedded GNU C Library: National Language (locale) da
...
+ii  mime-support             3.52-1+deb7u1     all               MIME files 'mime.types' & 'mailcap', and support prog
 ii  mount                    2.20.1-5.3        amd64             Tools for mounting and manipulating filesystems
 ii  multiarch-support        2.13-38+deb7u6    amd64             Transitional package to ensure multiarch compatibilit
...
+ii  nginx-common             1.6.2-5~bpo70+1   all               small, powerful, scalable web/proxy server - common f
+ii  nginx-light              1.6.2-5~bpo70+1   amd64             nginx web/proxy server (basic version)
...
+ii  openssl                  1.0.1e-2+deb7u13  amd64             Secure Socket Layer (SSL) binary and related cryptogr
...
+ii  python                   2.7.3-4+deb7u1    all               interactive high-level object-oriented language (defa
+ii  python-medusa            1:0.5.4-7         all               Framework for implementing asynchronous servers
+ii  python-meld3             0.6.5-3.1         amd64             An HTML/XML templating system for Python
+ii  python-minimal           2.7.3-4+deb7u1    all               minimal subset of the Python language (default versio
+ii  python-pkg-resources     0.6.24-1          all               Package Discovery and Resource Access using pkg_resou
+ii  python-support           1.0.15            all               automated rebuilding support for Python modules
+ii  python2.7                2.7.3-6+deb7u2    amd64             Interactive high-level object-oriented language (vers
...
+ii  sudo                     1.8.5p2-1+nmu1    amd64             Provide limited super user privileges to specific use
+ii  supervisor               3.0a8-1.1+deb7u1  all               A system for controlling process state
