...
describe 'minimum2scp/baseimage-buster' do
  ...
  context 'with env [APT_LINE=jp APT_HTTP_PROXY=http://x.x.x.x:3142/ DEFAULT_LANG=en_US.UTF-8, DEFAULT_TZ=UTC]' do
    ...
    describe file('/etc/apt/sources.list') do
      ...
      ## testing
      its(:content) { should_not match apt_line_re[true, 'deb',     'http://ftp.jp.debian.org/debian/', 'testing',        'main', 'contrib', 'non-free'] }
      its(:content) { should_not match apt_line_re[true, 'deb-src', 'http://ftp.jp.debian.org/debian/', 'testing',        'main', 'contrib', 'non-free'] }
      ## sid
      its(:content) { should_not match apt_line_re[true, 'deb',     'http://ftp.jp.debian.org/debian/', 'sid',            'main', 'contrib', 'non-free'] }
      its(:content) { should_not match apt_line_re[true, 'deb-src', 'http://ftp.jp.debian.org/debian/', 'sid',            'main', 'contrib', 'non-free'] }
      ## experimental
      its(:content) { should_not match apt_line_re[true, 'deb',     'http://ftp.jp.debian.org/debian/', 'experimental',   'main', 'contrib', 'non-free'] }
      its(:content) { should_not match apt_line_re[true, 'deb-src', 'http://ftp.jp.debian.org/debian/', 'experimental',   'main', 'contrib', 'non-free'] }
    end
  ...
  context 'with env [INSTALL_DOCKER_CE_CLI=yes]' do
    ...
    describe command('docker --version') do
      its(:stdout){ should start_with('Docker version 28.3.1, ') }
    end
  end
  ...
  context 'with env [INSTALL_NGINX=yes]' do
    ...
    describe package('nginx') do
      it { should be_installed.with_version('1.22.0-1~buster') }
    end
