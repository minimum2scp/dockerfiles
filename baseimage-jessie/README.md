...
```
% docker run --rm minimum2scp/debian-jessie:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20151011-31499-1k0qf8g.out
% docker run --rm minimum2scp/baseimage-jessie:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20151011-31499-f3vmpz.out
% diff -u /tmp/docker-diff.20151011-31499-1k0qf8g.out /tmp/docker-diff.20151011-31499-f3vmpz.out
--- /tmp/docker-diff.20151011-31499-1k0qf8g.out	2015-10-11 00:18:01.601623604 +0900
+++ /tmp/docker-diff.20151011-31499-f3vmpz.out	2015-10-11 00:18:03.049624375 +0900
@@ -9,8 +9,12 @@
...
+ii  libldap-2.4-2:amd64      2.4.40+dfsg-1+deb amd64             OpenLDAP libraries
...
+ii  libsasl2-2:amd64         2.1.26.dfsg1-13+d amd64             Cyrus SASL - authentication abstraction library
+ii  libsasl2-modules-db:amd6 2.1.26.dfsg1-13+d amd64             Cyrus SASL - pluggable authentication modules (DB)
