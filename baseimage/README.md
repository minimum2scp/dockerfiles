...
```
% docker run --rm minimum2scp/debian:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150504-26302-14c3n0t.out
% docker run --rm minimum2scp/baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150504-26302-4zg5su.out
% diff -u /tmp/docker-diff.20150504-26302-14c3n0t.out /tmp/docker-diff.20150504-26302-4zg5su.out
--- /tmp/docker-diff.20150504-26302-14c3n0t.out	2015-05-04 01:30:59.174749514 +0900
+++ /tmp/docker-diff.20150504-26302-4zg5su.out	2015-05-04 01:31:00.598748844 +0900
@@ -1,3 +1,7 @@
+[master 421820f] apt: use http://ftp.jp.debian.org/debian/ (changed by /opt/init-wrapper/pre-init.d/05-aptline)
+ 1 file changed, 34 insertions(+), 34 deletions(-)
+ rewrite apt/sources.list (73%)
...
+ii  curl                     7.42.1-1          amd64             command line tool for transferring data with URL synt
...
+ii  etckeeper                1.18.1            all               store /etc in git, mercurial, bzr or darcs
 ...
 ii  gcc-5-base:amd64         5.1.1-3           amd64             GCC, the GNU Compiler Collection (base package)
...
+ii  libcurl3:amd64           7.42.1-1          amd64             easy-to-use client-side URL transfer library (OpenSSL
+ii  libcurl3-gnutls:amd64    7.42.1-1          amd64             easy-to-use client-side URL transfer library (GnuTLS 
...
+ii  libffi6:amd64            3.1-2+b2          amd64             Foreign Function Interface library runtime
 ii  libgcc1:amd64            1:5.1.1-3         amd64             GCC support library
...
+ii  libgnutls-deb0-28:amd64  3.3.14-2          amd64             GNU TLS library - main runtime library
...
+ii  libssl1.0.0:amd64        1.0.2a-1          amd64             Secure Sockets Layer toolkit - shared libraries
 ii  libstdc++6:amd64         5.1.1-3           amd64             GNU Standard C++ Library v3
...
+ii  perl                     5.20.2-4          amd64             Larry Wall's Practical Extraction and Report Language
