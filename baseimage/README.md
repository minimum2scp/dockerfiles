...
```
% docker run --rm minimum2scp/debian:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20141218-16146-m4dp29.out
% docker run --rm minimum2scp/baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20141218-16146-plsxf7.out
% diff -u /tmp/docker-diff.20141218-16146-m4dp29.out /tmp/docker-diff.20141218-16146-plsxf7.out
--- /tmp/docker-diff.20141218-16146-m4dp29.out	2014-12-18 00:55:58.641582346 +0900
+++ /tmp/docker-diff.20141218-16146-plsxf7.out	2014-12-18 00:55:59.257585138 +0900
@@ -6,11 +6,16 @@
...
+ii  libkrb5-3:amd64          1.12.1+dfsg-16    amd64             MIT Kerberos runtime libraries
