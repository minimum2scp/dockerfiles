...
```
% docker run --rm minimum2scp/debian:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150606-23990-rxtm0k.out
% docker run --rm minimum2scp/baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150606-23990-1i6vzt.out
% diff -u /tmp/docker-diff.20150606-23990-rxtm0k.out /tmp/docker-diff.20150606-23990-1i6vzt.out
--- /tmp/docker-diff.20150606-23990-rxtm0k.out	2015-06-06 00:19:26.779166391 +0900
+++ /tmp/docker-diff.20150606-23990-1i6vzt.out	2015-06-06 00:19:28.103182331 +0900
@@ -8,8 +8,12 @@
...
+ii  curl                     7.42.1-2+b1       amd64             command line tool for transferring data with URL synt
...
+ii  libcurl3-gnutls:amd64    7.42.1-2+b1       amd64             easy-to-use client-side URL transfer library (GnuTLS 
