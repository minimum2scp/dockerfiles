...
```
% docker run --rm minimum2scp/debian:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150711-32503-170vgbq.out
% docker run --rm minimum2scp/baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150711-32503-ry52yi.out
% diff -u /tmp/docker-diff.20150711-32503-170vgbq.out /tmp/docker-diff.20150711-32503-ry52yi.out
--- /tmp/docker-diff.20150711-32503-170vgbq.out	2015-07-11 00:07:13.629643928 +0900
+++ /tmp/docker-diff.20150711-32503-ry52yi.out	2015-07-11 00:07:15.001638882 +0900
@@ -8,8 +8,12 @@
...
+ii  etckeeper                1.18.1            all               store /etc in git, mercurial, bzr or darcs
 ...
 ii  gcc-5-base:amd64         5.1.1-13          amd64             GCC, the GNU Compiler Collection (base package)
