...
```
% docker run --rm minimum2scp/debian:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150617-24758-6rtfn4.out
% docker run --rm minimum2scp/baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150617-24758-1vn0xhq.out
% diff -u /tmp/docker-diff.20150617-24758-6rtfn4.out /tmp/docker-diff.20150617-24758-1vn0xhq.out
--- /tmp/docker-diff.20150617-24758-6rtfn4.out	2015-06-17 00:43:00.748841076 +0900
+++ /tmp/docker-diff.20150617-24758-1vn0xhq.out	2015-06-17 00:43:02.100845354 +0900
@@ -8,8 +8,12 @@
...
+ii  libgnutls-deb0-28:amd64  3.3.15-6          amd64             GNU TLS library - main runtime library
