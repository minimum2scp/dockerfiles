...
```
% docker run --rm minimum2scp/debian:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150519-26604-1h6r4cj.out
% docker run --rm minimum2scp/baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150519-26604-c2lqgn.out
% diff -u /tmp/docker-diff.20150519-26604-1h6r4cj.out /tmp/docker-diff.20150519-26604-c2lqgn.out
--- /tmp/docker-diff.20150519-26604-1h6r4cj.out	2015-05-19 01:33:38.333341603 +0900
+++ /tmp/docker-diff.20150519-26604-c2lqgn.out	2015-05-19 01:33:39.669358288 +0900
@@ -9,8 +9,12 @@
...
+ii  etckeeper                1.18.1            all               store /etc in git, mercurial, bzr or darcs
 ...
 ii  gcc-4.9-base:amd64       4.9.2-17          amd64             GCC, the GNU Compiler Collection (base package)
...
+ii  sysvinit-core            2.88dsf-59.1      amd64             System-V-like init utilities
