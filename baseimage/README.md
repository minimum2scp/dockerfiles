...
```
% docker run --rm minimum2scp/debian:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150309-25911-18vblhp.out
% docker run --rm minimum2scp/baseimage:latest bash -c 'export LANG=C; export COLUMNS=120; dpkg -l' > /tmp/docker-diff.20150309-25911-eqei1s.out
% diff -u /tmp/docker-diff.20150309-25911-18vblhp.out /tmp/docker-diff.20150309-25911-eqei1s.out
--- /tmp/docker-diff.20150309-25911-18vblhp.out	2015-03-09 01:00:04.096156804 +0900
+++ /tmp/docker-diff.20150309-25911-eqei1s.out	2015-03-09 01:00:04.716152709 +0900
@@ -9,8 +9,12 @@
